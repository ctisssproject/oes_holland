...
$o_table = new View_Goods_Information ( $_GET ['useid'] );
if ($o_table->getUid () > 0) {
} else {
	echo ('<script type="text/javascript">location=\'goods_information_use.php\';</script>');
	exit ( 0 );
}
?>
...
<table class="main" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<form method="post" id="submit_form"
			action="include/bn_submit.svr.php?function=GoodsInformationUseCheck"
			enctype="multipart/form-data" target="ajax_submit_frame"
			style="width: 100%" onsubmit="this.submit();parent.parent.Common_OpenLoading();">
		<div class="list out">
		<div class="title"><div>审核资料寄送申请</div></div>
		<table border="0" cellpadding="0" cellspacing="0">
			...
			<tr class="bright">
				...
				<td class="right_none">
				<img align="absmiddle" style="width:200px;height:200px" src="<?php echo($o_table->getPhoto ( ))?>" alt="" /> <?php echo($o_table->getInfoName( ))?></span>
				</td>
			</tr>
			<tr class="dark">
				<td style="width: 100px"><span>资料介绍</span></td>
				<td class="right_none"><?php echo($o_table->getExplain())?></td>
			</tr>	
			<tr class="bright">
				<td style="width: 100px"><span>领用数量</span></td>
				<td class="right_none">				
				<input id="Vcl_Sum" name="Vcl_Sum" maxlength="200"
					value="<?php echo($o_table->getSum())?>" style="width: 240px;"type="text" /> <span class="red">*</span></td>
			</tr>
			<tr class="dark">
		...
		<div class="subButton" onclick="document.getElementById('submit_form').onsubmit();">批准</div>
		<div class="subButton2" onclick="goBack()">返回</div>
		</div>
		</div>
		<input type="hidden" name="Vcl_UseId" value="<?php echo($_GET['useid'])?>"/> 
		</form>
		</td>
	</tr>
</table>
