...
public function getNetDiskFileListForAttchment($n_folderid) {
       ...
       ';
		...
		for($i = 0; $i < $n_count; $i ++) {
			//如果是图片，双击后打开。
			$html .= '
			<div title="' . $o_file->getFilename ( $i ) . '" onclick="selectedFile(this,\'../../sub/service/download.php?fileid='.$o_file->getFileId ( $i ).'\',\''.$o_file->getFilename ( $i ).'\',\''.$o_file->getSuffix ( $i ).'\')" class="off">
	            <div>
