...
function OutputList() {
	$fp = fopen ( 'output/information.csv', 'w' );
	...
	for($i=0;$i<$n_count;$i++)
	{
		...
		SetTotalInfo ( $o_send->getName($i),$o_send->getCompany($i),$o_send->getDept($i),$o_send->getJob($i),$o_send->getAddress($i),$o_send->getPhone($i),$o_send->getUserName($i),$o_send->getInfoName($i),$fp );
	}
	fclose ( $fp );
}
