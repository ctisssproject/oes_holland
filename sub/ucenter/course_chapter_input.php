...
<table class="main" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<form method="post" id="submit_form"
			action="include/bn_submit.svr.php?function=CourseChapterInput"
		...
		<table border="0" cellpadding="0" cellspacing="0">
			...
			<tr class="bright">
				<td style="width: 80px"><span>行程分类</span></td>
				<td class="right_none"><select class="BigSelect" onchange="courseChangeType(this)">
				...
				for($i = 0; $i < $n_count; $i ++) {
					echo ('<option value="' . $o_table->getTypeId($i) . '">' . $o_table->getName($i) . '</option>');				
				}
				?>
				</select> <span class="red">*</span></td>
			</tr>
			<tr class="dark">
				<td style="width: 80px"><span>行程名称</span></td>
				<td class="right_none"><div id="title"><select class="BigSelect" name="Vcl_TitleId" id="Vcl_TitleId">
				<option value="0"></option>
				</select> <span class="red">*</span></div></td>
			</tr>
			<tr class="bright">
		...
		<div class="subButton" onclick="courseChapterInputSubmit()">导入</div>
		<div class="subButton2" onclick="goBack()">返回</div>
		</div>
		</div>
		<input type="hidden" name="Vcl_TermId" id="Vcl_TermId" value="<?php echo($_GET['termid'])?>" /></form>
		</td>
	</tr>
</table>
</div>
