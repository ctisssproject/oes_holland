...
function Common_OpenLoading(){
    var o_obj=document.getElementById('master_box');
    var a_arr=[];
    a_arr.push('<div class="loading">');
    a_arr.push('    <div class="background"><div></div></div>');
    a_arr.push('</div>');
	window.clearInterval(N_Dialog_TimeHandle);
	N_Dialog_Height=84
	N_Dialog_Width=338
...
function Common_OpenLoading_Dialog(){
    var o_obj=document.getElementById('master_box_loading');
    var a_arr=[];
    a_arr.push('<div class="loading">');
    a_arr.push('    <div class="background"><div></div></div>');
    a_arr.push('</div>');
...
function Dialog_GetFrame(s_title,s_message){
    ...
    a_arr.push('           </td>');
    a_arr.push('            <td class="title"><div class="background"></div><div class="content">'+s_message+'</div>');
    a_arr.push('            </td>');
    a_arr.push('        </tr>');
    a_arr.push('        <tr>');
    a_arr.push('            <td>&nbsp;');
    a_arr.push('            </td>');
    a_arr.push('            <td class="button"><div class="ok" onclick="Dialog_Ok_Button()">确定</div><div class="cancel" onclick="Common_CloseDialog()">取消</div>');
    a_arr.push('            </td>');
    a_arr.push('        </tr>');
    a_arr.push('    </table>');
    a_arr.push('</div>');
...
function Common_CloseDialog(){
    ...
    window.clearInterval(N_Dialog_TimeHandle);
    hideBox()
    setTimeout('$("#master_box_bj").hide()',300)	
	setTimeout('$("#master_box").hide()',300)
	setTimeout('$("#master_box_loading").hide()',300)
    //N_Dialog_Width=0
    //N_Dialog_Height=0    
    Enable_Vcl()//启用控件
}
