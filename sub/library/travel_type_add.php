...
<table class="main" border="0" cellpadding="0" cellspacing="0"
	...
	<tr>
		...
		<form method="post" id="submit_form"
			action="include/bn_submit.svr.php?function=TravelTypeAdd"
			enctype="multipart/form-data" target="ajax_submit_frame"
			style="width: 100%" onsubmit="this.submit();parent.parent.Common_OpenLoading();">
			<input type="hidden" name="Vcl_Url" value="<?php echo(str_replace ( substr( $_SERVER['PHP_SELF'] , strrpos($_SERVER['PHP_SELF'] , '/')+1 ), '', $_SERVER['PHP_SELF']))?>"/>
			<input type="hidden" name="Vcl_From" value="<?php echo($_GET['from'])?>"/>
		<div class="list out">
			<div class="title">
				<div>添加分类</div>
				<?php 
				...
				?>
				<div style="height:18px" class="subButton" onclick="travelTypeAddSubmit()">提交</div>
			</div>
		...
		<div class="subButton" onclick="travelTypeAddSubmit()">提交</div>
		<?php 
				if($_GET['from']==1)
				{
					echo('
