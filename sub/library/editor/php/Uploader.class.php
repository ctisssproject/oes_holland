...
"文件大小超出 upload_max_filesize 限制", "文件大小超出 MAX_FILE_SIZE 限制", "文件未被完整上传", "没有文件被上传", "上传文件为空", "POST" => "文件大小超出 post_max_size 限制", "SIZE" => "文件大小超出网站限制", "TYPE" => "不允许的文件类型", "DIR" => "目录创建失败", "IO" => "输入输出错误", "UNKNOWN" => "未知错误", "MOVE" => "文件保存时出错" );
	...
	private function upFile($base64) {
		...
		if ($this->stateInfo == $this->stateMap [0]) {
			...
			mkdir ( RELATIVITY_PATH . 'uploaddata/netdisk/' . $o_filename, 0700 ); //建立文件夹
			$this->fullName = RELATIVITY_PATH . 'uploaddata/netdisk/' . $o_filename . '/' . $o_filename . $this->fileType;
			if (! move_uploaded_file ( $file ["tmp_name"], $this->fullName )) {
				...
				$o_file->setPath ( '/uploaddata/netdisk/' . $o_filename );
				//设置文件后缀信息
				...
				if ($o_suffix->getClassName () != 'image') {
					...
					$this->fullName = RELATIVITY_PATH . 'sub/service/download.php?fileid='.$o_file->getFileId();
				}
			}
		}
	}
