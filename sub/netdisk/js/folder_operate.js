...
function deleteFolder(n_id)
{
    ...
    parent.Dialog_Confirm("真的要删除这个文件夹吗？",function (){o_ajax_request.SendRequest()});
}
...
function submitRenameFolder()
{
    ...
    if (s_temp.length==0)
    {
        parent.Dialog_Message("[ 文件夹名 ] 不能为空！")
        return
    }
...
function callbackCopyFolder(n_fileid,n_folderid)
{
    ...
    parent.Dialog_Confirm("目标目录中有相同文件夹！<br>是否替换？",function (){o_ajax_request.SendRequest()});
}
function callbackCopyFolderFinish(n_parent)
{
    ...
    try{
        parent.document.getElementsByTagName('frame')[0].contentWindow.refreshPathNoBold(n_parent)
    }catch(e){
...
function FolderNew(n_parentid){
    ...
    a_arr.push('		</tr>')
    a_arr.push('		<tr class="TableControl" align="center">')
    a_arr.push('			<td colspan="2" nowrap="nowrap"><input value="新建" class="submitButton" style="font-family:微软雅黑;"')
    a_arr.push('				onclick="submitRenameFolder()" type="button" /> &nbsp;&nbsp; <input')
    a_arr.push('				value="取消" class="submitButton2" style="font-family:微软雅黑;" onclick="Common_CloseDialog()"')
    a_arr.push('				type="button" /></td>')
    a_arr.push('		</tr>')
