...
function deleteFile(n_id)
{
    ...
    parent.Dialog_Confirm("真的要删除这个文件吗？",function (){o_ajax_request.SendRequest()});
}
//////////////////////////////////////////////////////////////////////单个文件重命名//////////////////////////////////////////////////////////////////////////////////
function submitRename()
{
    ...
    if (s_temp.length==0)
    {
        parent.Dialog_Message("[ 文件名 ] 不能为空！")
        return
    }
...
function fileRename(n_id,s_filename){
    ...
    a_arr.push('			<td colspan="2" nowrap="nowrap"><input value="重命名" class="submitButton" style="font-family:微软雅黑;"')
    a_arr.push('				onclick="submitRename()" type="button" /> &nbsp;&nbsp; <input')
    a_arr.push('				value="取消" class="submitButton2" style="font-family:微软雅黑;" onclick="Common_CloseDialog()"')
...
function showMoveDialog(n_id,s_tree){
    ...
    a_arr.push('		</tr>')
    a_arr.push('		<tr class="TableControl" align="center">')
    a_arr.push('			<td colspan="1" nowrap="nowrap"><input value="移动" class="submitButton" style="font-family:微软雅黑;"')
...
function callbackMoveFile(n_fileid,n_folderid)
{
    ...
    parent.Dialog_Confirm("目标目录中有相同文件名！<br>是否覆盖？",function (){o_ajax_request.SendRequest();});
}
//////////////////////////////////////////////////////////////////////单个文件复制////////////////////////////////////////////////////////////////////////////////
...
function showCopyDialog(n_id,s_tree){
    ...
    a_arr.push('		</tr>')
    a_arr.push('		<tr class="TableControl" align="center">')
    a_arr.push('			<td colspan="1" nowrap="nowrap"><input value="复制" class="submitButton" style="font-family:微软雅黑;"')
    a_arr.push('				type="submit" /> &nbsp;&nbsp; <input')
    a_arr.push('				value="取消" class="submitButton2" style="font-family:微软雅黑;" onclick="Common_CloseDialog()"')
    a_arr.push('				type="button" /></td>')
    a_arr.push('		</tr>')
...
function copyAndMoveAll(s_str)
{
    ...
    if (n_folderid=='' && n_fileid=='')
    {
        if (s_str=='copy')
        {
            parent.Dialog_Message('请选择要复制的文件！');
        }else{
            parent.Dialog_Message('请选择要移动的文件！');
        }        
        return
    } 
    var o_ajax_request=new AjaxRequest();
...
function showCopyAndMoveAllDialog(n_folderid,n_fileid,s_tree,s_type){
    ...
    a_arr.push('		</tr>')
    a_arr.push('		<tr class="TableControl" align="center">')
    a_arr.push('			<td colspan="1" nowrap="nowrap"><input value="'+s_str+'" class="submitButton" style="font-family:微软雅黑;" onclick="submitCopyAndMoveAll(2,\''+s_type+'\')"')
...
function callbackCopyAndMoveAll(old_filename,old_size,old_date,old_class,new_filename,new_size,new_date,new_class,s_folder_id,s_file_id,n_folder_start,n_file_start,n_folderid,s_type)
{
    ...
    a_arr.push('		</tr>')
    a_arr.push('		<tr class="TableControl" align="center">')
    a_arr.push('			<td colspan="1" nowrap="nowrap"><input value="覆盖" class="submitButton" style="font-family:微软雅黑;" onclick="submitCopyAndMoveAll(1,\''+s_type+'\')"')
    a_arr.push('				type="button" /> &nbsp;&nbsp;<input value="跳过" class="submitButton" style="font-family:微软雅黑;" onclick="submitCopyAndMoveAll(0,\''+s_type+'\')"type="button" /> &nbsp;&nbsp;<input')
    a_arr.push('				value="取消" class="submitButton2" style="font-family:微软雅黑;" onclick="Common_CloseDialog()"')
    a_arr.push('				type="button" /></td>')
    a_arr.push('		</tr>')
    a_arr.push('	</tbody>')
...
function deleteAll()
{
    ...
    if (n_folderid=='' && n_fileid=='')
    {
        parent.Dialog_Message('请选择要删除的文件！');     
        return
    } 
    var o_ajax_request=new AjaxRequest();
    ...
    o_ajax_request.PushParameter(n_folderid);
    o_ajax_request.PushParameter(n_fileid);
    parent.Dialog_Confirm("真的要删除这些文件吗？",function (){o_ajax_request.SendRequest()});
...
{//彻底删除文件和文件夹
    ...
    if (n_folderid=='' && n_fileid=='')
    {
        parent.Dialog_Message('请选择要彻底删除的文件！');     
        return
    } 
    var o_ajax_request=new AjaxRequest();
    ...
    o_ajax_request.PushParameter(n_folderid);
    o_ajax_request.PushParameter(n_fileid);
    parent.Dialog_Confirm("真的要彻底删除吗？",function (){o_ajax_request.SendRequest()});   
}
...
{//彻底删除单个文件夹
    ...
    o_ajax_request.setPage('include/it_ajax.svr.php');
    parent.Dialog_Confirm("真的清空回收站吗？",function (){o_ajax_request.SendRequest()});
}
function reduction(n_id,n_type)
{
    ...
    parent.Dialog_Confirm("真的还原吗？",function (){o_ajax_request.SendRequest()});
}
function reductionReplace(n_id,n_type)
{
    ...
    if(n_type==1)
    {
        parent.Dialog_Confirm("原目录有相同文件夹！<br>是否替换？",function (){o_ajax_request.SendRequest()});
    }else{
        parent.Dialog_Confirm("原目录有相同文件！<br>是否替换？",function (){o_ajax_request.SendRequest()});
    }
}
function reductionDefult(n_id,n_type)
{
    ...
    if(n_type==1)
    {
        parent.Dialog_Confirm("该文件夹的父目录已经不存在！<br>系统将把文件夹还原到<br>[ 回收站还原 ] 文件夹中！<br>是否同意？",function (){o_ajax_request.SendRequest()});
    }else{
        parent.Dialog_Confirm("该文件的父目录已经不存在！<br>系统将把文件还原到<br>[ 回收站还原 ] 文件夹中！<br>是否同意？",function (){o_ajax_request.SendRequest()});
    }
}