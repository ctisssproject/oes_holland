...
function openPathSend(o_obj,n_id,s_function)
{
    ...
    //如果已经有了，就隐藏否则读取子目录
        ...
        //如果已经隐藏，则显示，否则隐藏
            ...
            temp_obj[0].src='images/minus.gif'    
        }else{
            temp_obj[0].style.display='none'
            temp_obj=temp_obj[0].parentNode.getElementsByTagName('img')
            temp_obj[0].src='images/plus.gif'    
        }
...
function callbackOpenPath(s_name,s_id)
{
    ...
    if (s_name=='')
    {
        ...
        o_obj[0].src='images/none.png'
        //o_obj[0].parentNode.innerHTML='<img src="images/plus.gif" alt="" align="absmiddle" style="display:none"/>'+o_obj[0].parentNode.innerHTML
        return
    } 
    ...
    {
        a_arr.push('<li style="display:block;margin-top:4px">')
        ...
        a_arr.push('<img src="images/plus.gif" alt="" align="absmiddle" onclick="openPath(this,'+a_id[i]+')" />')
        ...
        a_arr.push(a_name[i])
        a_arr.push('</a>')
        a_arr.push('<img src="images/loading.gif" alt="" align="absmiddle" style="display:none"/>')
        a_arr.push('</span>')
        a_arr.push('</li>')
    }
    a_arr.push('</ul>')
...
function uploadMoreFile(n_id){
    ...
    a_arr.push('			    <!--<![endif]-->')
    a_arr.push('			    <param name="quality" value="high" />')
    a_arr.push('			    <param name="flashvars" value="N_ID='+n_id+'&S_Url=http://www.hollandtravelexpert.com/sub/netdisk/include/bn_submit.svr.php?function=UpLoadPicture"/>')
    a_arr.push('			    <param name="wmode" value="opaque" />')
    a_arr.push('			    <param name="swfversion" value="8.0.35.0" />	')											   
    a_arr.push('			    <param name="expressinstall" value="../../images/expressInstall.swf" />')
    ...
    a_arr.push('				value="关闭" class="submitButton" onclick="Common_CloseDialog()"')
    a_arr.push('				type="button" /></td>')
    a_arr.push('		</tr>')
    a_arr.push('	</tbody>')
    a_arr.push('</table>')
