...
class Operate extends Bn_Basic {
	...
	public function CopyFolderAndReplace($n_folderid, $n_folderidto, $n_uid) {
		...
		if ($o_file->getParentId () == $n_folderidto) {
			return;
		}
		//检测有无重名
		$o_file_2 = new Netdisk_Folder ();
	...
	public function UpLoadPicture($n_uid) {
		...
		$o_path = md5 ( $_FILES ['Filedata'] ['name'] . rand ( 0, 9999 ) . $this->GetTimeCut () );
		mkdir ( RELATIVITY_PATH . 'uploaddata/netdisk/' . $o_path, 0700 ); //建立文件夹
		$filename = $_FILES ['Filedata'] ['name']; //原文件名
		$this->FileName=$filename; //原文件名,供上传时替换使用
		$fileext = strtolower ( trim ( substr ( strrchr ( $filename, '.' ), 1 ) ) ); //扩展名
		$o_filename = $o_path . '.' . $fileext; //保存到服务器的文件名
		$o_to = RELATIVITY_PATH . 'uploaddata/netdisk/' . $o_path . '/' . $o_filename;
		//计算网盘空间
		$filesize = floor ( $_FILES ['Filedata'] ['size'] / 1024 );
		//在数据库中保存文件信息
		$o_file = new Netdisk_File ();
		$o_file->setFilename ( $this->getOriginalFileName($filename,$_GET ['id']) );
		$o_file->setOriginalFilename ( $o_filename );
		$o_file->setFilesize ( $filesize );
		$o_file->setPath ( '/uploaddata/netdisk/' . $o_path );
		//设置文件后缀信息
		$o_suffix = new Netdisk_Type ( $fileext );
		if ($o_suffix->getClassName () == false) {
			$o_file->setSuffix ( 'other' );
	...
	private function getOriginalFileName($s_name,$n_folder_id)
	{
		...
		if ($o_file->getAllCount()>0)
		{
			...
			$a_filename = explode ( ".", $this->FileName);
